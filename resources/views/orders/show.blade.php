...
@section('content')
    ...
    <div class="order-layout">
        ...
        <aside class="order-layout__aside">
            @if($order->payment_method === 'cod')
                @if($waOrderUrl)
                    <a href="{{ $waOrderUrl }}" target="_blank" rel="noopener" class="btn block">
                        Kirim pesanan via WhatsApp
                    </a>
                @else
                    <p class="toast toast--err">Nomor WhatsApp cabang belum dikonfigurasi.</p>
                @endif
                <p class="muted" style="margin-top:8px;font-size:0.85rem">Admin cabang akan memproses pesanan dan mengirim nota via WhatsApp.</p>
            @else
                @if($qrisUrl)
                    <div class="panel" style="text-align:center">
                        <p style="font-weight:700;margin:0 0 8px">Scan QRIS — Rp {{ number_format($order->grand_total, 0, ',', '.') }}</p>
                        <img src="{{ $qrisUrl }}" alt="QRIS" class="order-qris-img">
                    </div>
                @else
                    <p class="toast toast--err">QRIS cabang belum dikonfigurasi di sistem.</p>
                @endif
                ...
                @if(in_array($order->status, ['pending_transfer', 'proof_submitted'], true))
                    ...
                    @else
                        <form method="post" action="{{ route('orders.upload-proof', $order) }}" enctype="multipart/form-data" class="panel" style="margin-top:16px">
                            @csrf
                            <div class="field">
